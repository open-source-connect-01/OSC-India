  ...
  const handleOAuthSignIn = async (provider: "github" | "google") => {
    ...
    if (res?.error) {
      if (res.error.toLowerCase().includes("provider is not enabled") || res.error.toLowerCase().includes("unsupported provider")) {
        setErrorMessage(`${provider === "google" ? "Google" : "GitHub"} sign-in is not enabled in your Supabase project. Please enable it in the Supabase Dashboard under Authentication → Providers.`);
      } else {
    ...
    >
      {profile === undefined ? (
        ...
        <>
          ...
          {errorMessage && (
            <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#ef4444", padding: "12px 14px", borderRadius: "10px", fontSize: "13px", lineHeight: "1.5", marginBottom: "20px" }}>
              {errorMessage}
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <button 
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-3 w-full bg-[#1c1c1f] hover:bg-[#252529] text-white text-[14px] font-medium rounded-xl border border-[rgba(255,255,255,0.12)] transition-all cursor-pointer disabled:opacity-60 hover:border-[rgba(255,255,255,0.25)]"
              style={{ padding: "16px" }}
            >
            ...
            <button 
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-3 w-full bg-white hover:bg-gray-100 text-gray-900 text-[14px] font-medium rounded-xl transition-all cursor-pointer disabled:opacity-60 shadow-md"
              style={{ padding: "16px" }}
            >
              {loadingProvider === "google" ? (
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <div style={{ width: "18px", height: "18px", border: "2px solid rgba(0,0,0,0.2)", borderTopColor: "#4285F4", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
                  <span style={{ color: "#1f2937", fontWeight: 600 }}>Connecting to Google...</span>
                </div>
              ) : (
                <>
